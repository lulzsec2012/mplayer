...
#define MB_NUM_W (FRAME_WIDTH/16)
#define MB_NUM_H (FRAME_HEIGHT/16)
    typedef struct{
      int modeY;
      int modeUV;
    } dec_pred_mode;
    dec_pred_mode * pdec_pred_mode;
    pdec_pred_mode = (dec_pred_mode *)malloc(sizeof(dec_pred_mode)*MB_NUM_W*MB_NUM_H);
    if(!pdec_pred_mode){
      printf("pdec_pred_mode==NULL!");
      exit(1);
    }
    //~add by lzlu
    if( h->pps.cabac ) {
...
//STOP_TIMER("decode_mb_cabac")
	    ...
	    //add by lzlu
	    if(h->slice_type_nos == AV_PICTURE_TYPE_I && 0){
	      if(h->intra16x16_pred_mode < 0){
		(pdec_pred_mode+h->s.mb_x+h->s.mb_y*MB_NUM_W)->modeY = 5;
	      }else{
		(pdec_pred_mode+h->s.mb_x+h->s.mb_y*MB_NUM_W)->modeY = h->intra16x16_pred_mode;
	      }
	      (pdec_pred_mode+h->s.mb_x+h->s.mb_y*MB_NUM_W)->modeUV = h->chroma_pred_mode;
	      if(h->s.mb_x==MB_NUM_W-1 && h->s.mb_y==MB_NUM_H-1){
		FILE * fp = fopen("intraPredMode.txt","w+");
		if(fp==NULL){
		  printf("open intraPredMode.txt failed");
		}else{
		  fwrite(pdec_pred_mode,sizeof(dec_pred_mode),MB_NUM_W*MB_NUM_H,fp);
		  fclose(fp);
		}
	      }
	    }
